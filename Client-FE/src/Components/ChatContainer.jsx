...
const ChatContainer = () => {
  ...
  const { messages, getMessages, selectedUser, setSelectedUser, sendMessage,menuOpen,setMenuOpen } = useContext(ChatContext)
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg '>
      ...
      {/* Chat Area --------------------------------------------------------------------*/}
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6 border-red-500 '>
        {messages.map((msg, index) => (
          <div key={index}
            className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
            {msg.image
              ? (<img src={msg.image} alt='ImageAlt' className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8' />)
            ...
            <div className='text-center text-xs'>
              <img
                ...
                className='w-7 rounded-full border-red-800'
              />
      ...
      {/*-------------- Bottom Area where INput is Done--------- */}
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full '>
          ...
          />
          <input type="file" onChange={handleSendImage} id='image' accept='image/png, image/jpg' hidden />
          <label htmlFor="image">
